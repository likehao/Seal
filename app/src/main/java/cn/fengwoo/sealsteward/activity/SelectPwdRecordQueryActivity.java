...
import cn.fengwoo.sealsteward.bean.StampRecordDatax;
 ...
 * 查询到的密码盖章记录
 */
public class SelectPwdRecordQueryActivity extends BaseActivity implements View.OnClickListener {
...
//        sealId = intent.getStringExtra("sealId");
        ...
        getPwdRecord();
        adapter = new PwdRecordAdapter(list, SelectPwdRecordQueryActivity.this);
    ...
    private void getPwdRecordData(RefreshLayout refreshLayout) {
        StampRecordDatax stampRecordData = new StampRecordDatax();
...
//        param.Param(applyUser, endTime, sealId, 2,startTime);  // type 1.APP盖章   2.密码盖章
        ...
        StampRecordDatax.ParamBean paramBean = new StampRecordDatax.ParamBean();
        paramBean.setSealId(sealId);
        paramBean.setUserId(userId);
        paramBean.setUserNumber(Integer.parseInt(userNumber));
        paramBean.setUserType(Integer.parseInt(userType));
        stampRecordData.setParam(paramBean);

        HttpUtil.sendDataAsync(this, HttpUrl.USER_OFFLINE_STAMP_RECORD, 2, null, stampRecordData, new Callback() {
            @Override
